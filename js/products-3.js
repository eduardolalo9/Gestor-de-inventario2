...
export function handleFileImport(event) {
    ...
    if (!file) return;
    // BUG-FIX: XLSX se carga con defer; puede no estar listo si el usuario importa muy rápido
    if (!window.XLSX) {
        showNotification('⏳ La librería Excel aún carga, espera 2 segundos e intenta de nuevo');
        event.target.value = '';
        return;
    }
    ...
    pesoBotellaLlenaOz: ['PesoBotellaOz','pesoBotellaOz',
            ...
            jsonData.forEach(row => {
                ...
                // BUG-FIX: registrar el stock en inventarioConteo para que
                // syncStockByAreaFromConteo() no lo resetee a 0 en el próximo arranque
                if (stock > 0) {
            ...
            // BUG-FIX: recalcular stockByArea desde inventarioConteo recién creado
            syncStockByAreaFromConteo();
