...
function _loadXLSX() {
    return new Promise((resolve, reject) => {
        ...
        s.onload  = () => { if (window.XLSX) resolve(window.XLSX); else reject(new Error('SheetJS cargó pero window.XLSX es undefined')); };
        s.onerror = () => reject(new Error('No se pudo cargar la librería Excel. Verifica tu conexión.'));
...
export function handleFileImport(event) {
    ...
    // ── Alias de columnas normalizados (SOLO coincidencia EXACTA tras _norm) ──
    // Esto evita falsos positivos por substring: 'capacidadml' no se confunde con 'id'
    const columnMap = {
        id:   ['id', 'codigo', 'clave', 'sku', 'cod', 'num'],
        name: ['producto', 'nombre', 'descripcion', 'articulo',
               'item', 'name', 'description', 'detalle', 'concepto', 'bebida'],
        unit: ['unidad', 'medida', 'unit', 'um'],
        group:['grupo', 'categoria', 'group', 'category', 'familia'],
        stock:['cantidad', 'stock', 'enteras', 'existencia', 'qty', 'quantity'],
        capacidadMl:        ['capacidadml', 'capacidad_ml', 'capml', 'capacidad'],
        pesoBotellaLlenaOz: ['pesobotella0z',     // ← tu Excel usa 0 (cero), no O letra
                             'pesobotellaoz',
                             'pesollenaoz',
                             'pesobotella_oz',
                             'pesobotellaoz',
    ...
    // ── Mapea cada columna del Excel a un campo conocido (solo match exacto) ──
    function _mapHeaders(rowKeys) {
        const mapped = {};
        rowKeys.forEach(rk => {
            if (rk === null || rk === undefined) return;  // ignorar columnas sin header
            const rkNorm = _norm(rk);
            if (!rkNorm) return;
            for (const [field, aliases] of Object.entries(columnMap)) {
                if (mapped[field]) continue;          // ya encontrado, no sobreescribir
                if (aliases.includes(rkNorm)) {        // SOLO coincidencia exacta
                    mapped[field] = rk;
                    break;
                }
            }
    ...
    _loadXLSX().then(XLSX => {
        ...
        reader.onload = function(e) {
            try {
                ...
                if (!firstSheet) {
                    showNotification(`❌ La hoja "${sheetName}" está vacía`); return;
                }

                // defval:'' evita celdas undefined en filas con columnas vacías
                const jsonData = XLSX.utils.sheet_to_json(firstSheet, { defval: '' });
                if (!jsonData?.length) {
                    showNotification('❌ El archivo no tiene datos — revisa que la fila 1 tenga encabezados'); return;
                }

                // ── Detectar columnas ────────────────────────────────────────
                const rowKeys   = Object.keys(jsonData[0]).filter(k => k && k !== '__EMPTY' && !k.startsWith('__EMPTY'));
                const headerMap = _mapHeaders(rowKeys);

                console.info('[Import] Columnas del Excel:', rowKeys);
                console.info('[Import] Campos detectados:', headerMap);

                if (!headerMap.name) {
                    const visibles = rowKeys.slice(0, 8).join(' | ');
                    showNotification(
                        `❌ No se encontró columna de nombre de producto.\n` +
                        `Columnas en tu Excel: ${visibles}\n` +
                        `La columna debe llamarse: Producto, Nombre, Descripción, Artículo o Bebida`
                    );
                    return;
                }
                ...
                jsonData.forEach((row, rowIdx) => {
                    ...
                    // ID: usar el del Excel (puede ser número), o autogenerar
                    const rawId = headerMap.id ? row[headerMap.id] : '';
                    ...
                    // CapacidadML y PesoBotella: 0 = no configurado (tratar como null)
                    const capRaw  = headerMap.capacidadMl        ? row[headerMap.capacidadMl]        : '';
                    const pesoRaw = headerMap.pesoBotellaLlenaOz ? row[headerMap.pesoBotellaLlenaOz] : '';
                    const capNum  = parseFloat(capRaw);
                    const pesoNum = parseFloat(pesoRaw);
                    // Solo asignar si el valor es un número POSITIVO (0 significa "no aplica")
                    const capacidadMl        = (!isNaN(capNum)  && capNum  > 0) ? capNum  : null;
                    const pesoBotellaLlenaOz = (!isNaN(pesoNum) && pesoNum > 0) ? pesoNum : null;
                    ...
                    // Registrar stock en inventarioConteo para que persista al reiniciar
                    if (stock > 0) {
                ...
                if (toImport.length === 0) {
                    showNotification(
                        ...
                        `Columna detectada: "${headerMap.name}" — verifica que las celdas no estén vacías`
                    );
                    return;
                }
                ...
                // Ir a pestaña Productos para ver lo importado
                state.activeTab    = 'productos';
                state.selectedGroup = 'Todos';
                state.searchTerm   = '';
                state.selectedArea = 'almacen';
                saveToLocalStorage();
                import('./render.js').then(m => m.switchTab('productos'));

            } catch (err) {
                showNotification('❌ Error al leer el archivo: ' + err.message);
                console.error('[Import Excel]', err);
            } finally {
                fileInput.value = '';   // permite re-importar el mismo archivo
            }
        };
