 ...
 *
 * FIX PRINCIPAL: El botón "Importar Excel" ya no depende de un
 * <input type="file"> persistente en el DOM. Usa una función global
 * que crea el input al vuelo, garantizando que el listener SIEMPRE
 * esté conectado a handleFileImport.
 */
...
/* ══════════════════════════════════════════════════════════════
   FIX: Función global para importar Excel.
   Crea un <input type="file"> temporal, escucha su cambio
   y llama a handleFileImport directamente.
   Esto elimina la dependencia de un input persistente en el DOM.
   ══════════════════════════════════════════════════════════════ */
window._importExcelFromButton = function () {
    ...
    input.addEventListener('change', function (e) {
        ...
        // Limpiar el input temporal del DOM
...
function renderInicioTab() {
    ...
    /* Botones admin: + Producto e Importar Excel */
    if (isAdmin) {
        ...
        /* ═══════════════════════════════════════════════════════
           FIX PRINCIPAL: El botón ahora llama a _importExcelFromButton()
           que crea un input temporal al vuelo.
           Ya NO depende de document.getElementById('fileInput').
           ═══════════════════════════════════════════════════════ */
        html += `<button onclick="window._importExcelFromButton()"
    ...
    html += '</div>'; /* cierra barra de filtros */
...
function renderAuditoriaConteo() {
    ...
    html += `<div class="mb-3 flex gap-2">
               ...
               <select onchange="window.updateSelectedGroup(this.value)"
                       class="px-2 py-2 bg-white text-gray-900 border border-gray-200 rounded-lg
        ...
        if (pending.length === 0) {
            ...
            pending.forEach(a => {
                html += `<div style="background:var(--surface);border:1px solid var(--border);
                           ...
                           · ${escapeHtml(a.campoLabel)}: ${escapeHtml(a.valorAnterior || '—')}
                           → ${escapeHtml(a.valorNuevo)}
                           <span style="margin-left:6px;font-size:0.65rem;color:var(--amber);">
                             ⏳ Pendiente de subir
                           </span>
                         </div>`;
            });
