...
import { AREAS, AREAS_AUDITORIA, AREAS_AUDITORIA_ICONS, AREAS_AUDITORIA_FA, AREA_KEYS } from './constants.js';
import { escapeHtml, updateHeaderActions } from './ui.js';
import { saveToLocalStorage } from './storage.js';
import { filterByGroup, getAvailableGroups, getTotalStock, calcularTotalConAbiertas, tieneConversion, generateProductId } from './products.js';
import { renderAuditTrailForProduct, renderAuditUserPanel, renderAuditComparePanel, auditoriaTotalAreasCompletadas, auditoriaTodasCompletas } from './audit.js';
...
export function switchTab(tab) {
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        ...
        if (btn.dataset.tab === tab) {
            btn.classList.remove('text-gray-600'); btn.classList.add('text-gray-900');
            if (!indicator) { const div=document.createElement('div'); div.className='tab-indicator absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 to-orange-500 rounded-t-full animate-slideIn'; btn.appendChild(div); }
        } else {
            btn.classList.remove('text-gray-900'); btn.classList.add('text-gray-600');
...
function renderPedidosTab() {
    ...
    filtered.forEach(p => {
        ...
        html += `<div class="flex-1"><span class="text-sm font-medium text-gray-900">${escapeHtml(p.name)}</span> <span class="text-xs text-gray-500">(stock: ${stock.toFixed(2)} ${escapeHtml(p.unit||'')})</span></div>`;
        html += `<button onclick="window.addToCart('${escapeHtml(p.id)}')" class="px-2 py-1 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded text-xs font-semibold">+ Agregar</button>`;
        html += `</div>`;
    });
    html += '</div>';
    ...
    if (state.orders.length > 0) {
        ...
        state.orders.forEach((order, idx) => {
            ...
            if (order.note) html += `<div class="mt-2 p-2 bg-purple-50 rounded"><p class="text-xs text-gray-600">Nota: ${escapeHtml(order.note)}</p></div>`;
            html += '</div>';
        });
        html += '</div>';
    }
...
function renderInventarioTab() {
    if (state.auditoriaView === 'counting' && state.auditoriaAreaActiva) return renderAuditoriaConteo();
...
function renderAuditoriaSeleccion() {
    ...
    html += '</div>';
    html += '<button onclick="window.auditoriaResetear()" title="Iniciar nueva auditoría" style="flex-shrink:0;padding:6px 10px;border-radius:var(--r-md);background:var(--red-dim);border:1px solid rgba(239,68,68,0.18);color:var(--red-text);font-size:0.7rem;font-weight:600;cursor:pointer;" class="flex items-center gap-1"><svg width="12" height="12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg> Nueva auditoría</button>';
    html += '</div>';
    html += '<div class="flex items-center gap-3"><div style="font-size:0.68rem;font-weight:600;color:var(--txt-muted);white-space:nowrap;">' + totalCompletas + ' / 3 áreas</div>';
    html += '<div class="audit-progress-bar" style="flex:1;"><div class="audit-progress-fill" style="width:' + porcentaje + '%;"></div></div>';
    html += '<div style="font-size:0.68rem;font-weight:700;color:' + (todasCompletas?'var(--green)':'var(--accent)') + ';white-space:nowrap;">' + porcentaje + '%</div></div></div>';
    // Panel de usuario + comparación
    ...
    ['almacen','barra1','barra2'].forEach(area => {
        ...
        html += '<div class="audit-area-card' + (isCompleta?' completada':'') + '" onclick="window.auditoriaEntrarArea(\'' + area + '\')" role="button" tabindex="0" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();window.auditoriaEntrarArea(\'' + area + '\')}">';
        html += '<div class="audit-area-icon"><i class="' + AREAS_AUDITORIA_FA[area] + '"></i></div>';
        html += '<div class="audit-area-info"><div class="audit-area-name">' + AREAS_AUDITORIA[area] + '</div>';
        html += '<span class="audit-area-status ' + (isCompleta?'completada':'pendiente') + '">' + (isCompleta?'✓ Completada':'⏳ Pendiente') + '</span></div>';
        html += '<svg class="audit-area-arrow w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>';
        html += '</div>';
    });
    html += '</div>';
...
function renderAuditoriaConteo() {
    ...
    html += '<div class="flex items-center gap-2 mb-2"><button onclick="window.auditoriaVolverSeleccion()" class="audit-back-btn"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"/></svg>Volver</button>';
    html += '<span class="audit-count-area-badge"><i class="' + AREAS_AUDITORIA_FA[area] + '"></i>' + areaLabel + '</span></div>';
    html += '<p style="font-size:0.7rem;color:var(--txt-muted);">Conteo ciego · ' + filtered.length + ' productos</p>';
    html += '</div>';
    // Búsqueda
    html += '<div class="mb-3"><input type="text" placeholder="Buscar..." value="' + escapeHtml(state.searchTerm) + '" oninput="window.updateSearchTerm(this.value)" class="w-full px-3 py-2 bg-white text-gray-900 border border-gray-200 rounded-lg text-sm"></div>';
    // Tarjetas de producto
    html += '<div class="space-y-2">';
    filtered.forEach(p => {
        const conteo   = state.auditoriaConteo[p.id]?.[area] || { enteras: 0, abiertas: [] };
        const hasCount = conteo.enteras > 0 || (conteo.abiertas && conteo.abiertas.length > 0 && conteo.abiertas.some(a=>a>0));
        html += '<div class="bg-white rounded-xl p-3 shadow-md border ' + (hasCount?'border-green-400/40':'border-transparent') + '" onclick="window.openInventarioModal(\'' + escapeHtml(p.id) + '\')" style="cursor:pointer;">';
        html += '<div class="flex items-center justify-between">';
        html += '<div class="flex-1 min-w-0"><span class="font-medium text-sm text-gray-900">' + escapeHtml(p.name) + '</span><div class="text-xs text-gray-500 mt-0.5">' + escapeHtml(p.group||'General') + ' · ' + escapeHtml(p.unit||'') + '</div></div>';
        html += '<div class="flex-shrink-0 ml-2">';
        if (hasCount) { html += '<div class="text-right"><div class="text-sm font-bold text-green-600">' + conteo.enteras + ' ent.</div>'; if (conteo.abiertas&&conteo.abiertas.some(a=>a>0)) html += '<div class="text-xs text-amber-500">+ ' + conteo.abiertas.filter(a=>a>0).length + ' ab.</div>'; html += '</div>'; }
        else { html += '<span class="text-xs text-gray-400 italic">Sin contar</span>'; }
        html += '</div></div>';
        html += renderAuditTrailForProduct(p.id, area);
        html += '</div>';
    });
    html += '</div>';
    html += '<button onclick="window.auditoriaFinalizarConteo()" class="audit-finish-btn"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>Finalizar conteo de ' + areaLabel + '</button>';
    html += '</div>';
    return html;
}
...
function renderHistoriaTab() {
    let html = '<div class="space-y-4">';
    // Selector de área e inventario actual
    html += '<div class="bg-white rounded-xl p-4 shadow-md">';
    html += '<h3 class="text-lg font-bold text-gray-900 mb-3">Guardar conteo actual</h3>';
    html += '<div class="flex gap-2 flex-wrap mb-3">';
    AREA_KEYS.forEach(area => {
